...
with col_main:
    # الترحيب
    if gender == "أنثى":
        welcome_msg, sub_text = f"أهلاً بكِ يا مهندسة {name} ✨", "دعينا نخطط لكِ مساراً يوافق تطلعاتكِ."
    else:
        welcome_msg, sub_text = f"أهلاً بك يا مهندس {name} ✨", "دعنا نخطط لك مساراً يوافق تطلعاتك."
    
    st.markdown(f'''<div class="welcome-card"><h3>{welcome_msg}</h3><p>{sub_text}</p></div>''', unsafe_allow_html=True)
    ...
    if st.button("تحديث وتحليل المسار"):
        ...
        if event['start'] <= sim_hour < event['end']:
            container.success("✅ متاح الآن: استمتع بالزيارة!")
        else:
            container.warning(f"⏳ مغلق: يفتح من {event['start']}:00 إلى {event['end']}:00")

    # النجوم
    st.markdown("---")
    st.subheader("⭐ تقييم تجربتك")
    star_rating = st.select_slider("التقييم:", options=[1, 2, 3, 4, 5], value=5)
