...
# 2. قاموس اللغات الشامل (Dictionary) - تم حذف أي إشارة للاسم
TRANSLATIONS = {
    "ar": {
...
# 3. قاعدة البيانات ثنائية اللغة (لحل عيب خلط اللغات)
PLACES_DB = {
    "ar": {
        "اقتصادية": [
        ...
        "فاخرة": [
            ...
            {"الوجهة": "مطل البجيري", "الفئة": "مطاعم ومقاهي", "وصف": "إطلالة تاريخية على حي الطريف.", "base_time": 18, "metro": True}
        ]
    },
    "en": {
        "Economy": [
        ...
        "Luxury": [
            ...
            {"الوجهة": "Bujairi Terrace", "الفئة": "Dining", "وصف": "Premium views of At-Turaif district.", "base_time": 18, "metro": True}
        ]
    }
}
...
st.markdown(f'''
    ...
    .stButton>button {{ background: linear-gradient(90deg, #0284C7 0%, #38BDF8 100%) !important; color: white !important; border-radius: 15px !important; font-weight: bold !important; width: 100%; }}
    </style>
...
if st.session_state.page == 'welcome':
    ...
    with st.form("welcome_form"):
        ...
        if st.form_submit_button(T["start_btn"]):
            # تحويل الاختيار لقيمة ثابتة للبحث في الداتابيز
...
# --- الصفحة الثانية ---
    ...
    with col_main:
        ...
        if st.button(T["analyze"]):
            # جلب الداتابيز حسب اللغة الحالية والميزانية
            db = PLACES_DB[st.session_state.lang][st.session_state.user_budget]
            st.session_state.suggestions = [random.choice(db)] # مثال للتحليل
