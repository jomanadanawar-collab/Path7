...
# 4. التنسيق البصري (إصلاح أزرار النجوم "المستفزة")
st.markdown(f'''
    <style>
    ...
    * {{ font-family: 'IBM Plex Sans Arabic', sans-serif !important; direction: {"rtl" if IS_AR else "ltr"}; }}
    ...
    .glass-card {{ background: rgba(255, 255, 255, 0.75); backdrop-filter: blur(12px); padding: 25px; border-radius: 25px; border: 1px solid rgba(255, 255, 255, 0.3); box-shadow: 0 15px 35px rgba(0,0,0,0.1); margin-bottom: 20px; text-align: {"right" if IS_AR else "left"}; }}
    ...
    /* أزرار النجوم المربعة والشفافة والبسيطة */
    div[data-testid="stHorizontalBlock"] button[key^="s"] {{
        width: 50px !important; 
        height: 50px !important; 
        padding: 0px !important;
        border-radius: 5px !important; /* شكل مربع كلاسيكي */
    ...
    div[data-testid="stHorizontalBlock"] button[key^="s"]:hover {{
    ...
    with col_m:
        ...
        if st.session_state.suggestions:
            ...
            for p in st.session_state.suggestions:
                display_name = p["الوجهة"] # يمكن إضافة ترجمة هنا لاحقاً
                display_desc = p["وصف"]
