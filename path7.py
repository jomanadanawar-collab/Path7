...
# 3. تنسيق CSS (إجبار النمط الفاتح + توضيح دائرة التقييم + تنسيق البوكس)
st.markdown('''
    <style>
    ...
    /* تنسيق البوكس الترحيبي الكحلي (يحتوي كل شيء) */
    .letter-container {
    ...
    /* إبراز دائرة السلايدر (التقييم) ومنع تداخلها */
    div[data-baseweb="slider"] > div > div { background-color: #1A365D !important; }
    div[role="slider"] { 
...
# 4. إدارة الحالة (Session State) لمنع تغير البيانات عند التقييم
...
# --- المشهد الأول: الرسالة (كل شيء داخل البوكس الكحلي) ---
if st.session_state.page == 'welcome':
    ...
    st.markdown('<p style="font-size: 1.2em; color: #4A5568; line-height: 1.6;">مرحباً بك في دليلك الذكي لاستكشاف الرياض.<br>فضلاً أدخل تفضيلاتك أدناه لنبدأ التحليل:</p>', unsafe_allow_html=True)
    st.markdown('<hr style="border: 0.5px solid #EDF2F7; margin: 20px 0;">', unsafe_allow_html=True)
    
    # المدخلات داخل البوكس
    ...
    if st.button("توليد المسار اللحظي 🚀", use_container_width=True):
        ...
        st.session_state.dest_result = None # تصفير النتيجة السابقة
        st.rerun()
    st.markdown('</div>', unsafe_allow_html=True)

# --- المشهد الثاني: صفحة التوافق اللحظي والنتائج ---
else:
    # هيدر النتائج مع زر تعديل أنيق
    ...
    with col_main:
        ...
        # زر التحديث (يغير النتائج فقط عند الضغط عليه)
        ...
        st.markdown("---")
        # التقييم (دائرة واضحة + تغييرها لا يغير الزحمة)
    ...
    with col_stats:
        ...
        t_display = f"{st.session_state.traffic_val}%" if st.session_state.traffic_val else "--"
        st.metric("الازدحام اللحظي", t_display)
