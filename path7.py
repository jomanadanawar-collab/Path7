...
# 1. قاعدة بيانات الوجهات (تأكدي من الأقواس)
riyadh_destinations = {
    "تاريخ": {
...
with st.sidebar:
    ...
    # اختيار الساعة (تلقائياً يأخذ ساعة النظام في السحابة)
    h = datetime.now().hour
    sim_hour = st.select_slider("ساعة الزيارة:", options=list(range(24)), value=h)
